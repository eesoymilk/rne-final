...
class Robot:
    ...
    def set_left_motor(self, value):
        ...
        jsonStr = json.dumps(
            {'leftMotor': left_ang, 'rightMotor': 0.0, 'flag': 1}
        )
    ...
    def set_right_motor(self, value):
        ...
        jsonStr = json.dumps(
            {'leftMotor': 0.0, 'rightMotor': right_ang, 'flag': 2}
        )
        self.ws.send(jsonStr)
    ...
    def set_motor(self, value_l, value_r):
        ...
        jsonStr = json.dumps(
            {'leftMotor': left_ang, 'rightMotor': right_ang, 'flag': 4}
        )
        self.ws.send(jsonStr)
    ...
    def add_motor(self, value_l, value_r):
        ...
        right_ang = self._move_to_wheel(value_r)
        jsonStr = json.dumps(
            {'leftMotor': left_ang, 'rightMotor': right_ang, 'flag': 3}
        )
        self.ws.send(jsonStr)
